 ...
 * disk per the 3a versioning design, so that's the real footprint. Phase 7c added
 * a short Redis cache (fail-open, same stance as lib/rate-limit.ts) - this used to
 * be live-computed on every call ("dataset size doesn't warrant it yet"), which is
 * still true for correctness, just not free to recompute on every dashboard load.
 */
export async function GET(req: NextRequest) {
    try {
        ...
        const data = await withCache('dashboard:summary', CACHE_TTL_SECONDS, computeSummary);
