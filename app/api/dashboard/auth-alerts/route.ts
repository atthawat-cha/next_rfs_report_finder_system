 ...
 * `login_failed` activity_logs rows within the window. Phase 7c added a short
 * Redis cache (fail-open) - a real brute-force in progress is still visible
 * within CACHE_TTL_SECONDS, which is an acceptable trade for not re-scanning
 * activity_logs on every dashboard load.
 */
export async function GET(req: NextRequest) {
    try {
        ...
        const data = await withCache(`dashboard:auth-alerts:${hours}`, CACHE_TTL_SECONDS, () =>
            computeAuthAlerts(hours)
        );
...
async function computeAuthAlerts(hours: number) {
