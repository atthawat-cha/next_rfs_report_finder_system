      ...
      <DialogContent className="sm:max-w-md">
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2">
            <AlertTriangle className="h-5 w-5 text-destructive" /> ลบเมนู
          </DialogTitle>
          <DialogDescription>
            ลบ &ldquo;{menu?.catagory_label ?? menu?.menu_label}&rdquo; ({menu?.group_label})
          </DialogDescription>
        </DialogHeader>

        {loading && (
          <div className="flex items-center text-muted-foreground text-sm">
            <Loader2 className="h-4 w-4 animate-spin mr-2" /> กำลังตรวจสอบผลกระทบ...
          </div>
        )}

        {!loading && counts && (
          <p className="text-sm">
            การลบนี้จะลบ <strong>{counts.permissions_count}</strong> permission และ{" "}
            <strong>{counts.role_permissions_count}</strong> role_permissions (สิทธิ์ของทุก role ที่เคยกำหนดไว้บนเมนูนี้)
            ไปด้วย — กู้คืนไม่ได้
          </p>
        )}
