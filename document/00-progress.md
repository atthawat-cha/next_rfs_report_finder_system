...
**Phase 5 วางแผนแล้ว (2026-08-19), เริ่มลงมือแล้ว** — โจทย์มาจาก `document/diff_req.md` ที่ผู้ใช้ commit เข้ามาเอง (`beb62ee`) แผนเต็มอยู่ที่ [`phase5-plan.md`](./phase5-plan.md) แบ่ง 6 sub-phase (5a-5f) ตัดสินใจครบทุกข้อกับผู้ใช้แล้วก่อนเขียน (ดูหัวข้อ "Resolved decisions" ในแผน) — **5a, 5b, 5c, 5d ปิดจบแล้ว (2026-08-19)** ดูรายละเอียดใต้ตาราง Phase 5 ด้านล่าง (5b เจอว่าแผนเดิมเข้าใจผิดว่า per-report ACL "zero UI" — ที่จริงมี inline UI มาตั้งแต่ 2d แล้ว; 5c เจอบั๊กจริง 2 ชั้นใน `buildRolePermissionInsert` ที่ทำให้ submenu-level permission grant ไม่เคยบันทึกได้เลยตั้งแต่ 2d แก้แล้ว; 5d เจอว่า `dialog-drawer.tsx`/`right-drawer.tsx` (ทั้งคู่) เป็น scaffold ที่ใช้งานจริงไม่ได้ — ดูรายละเอียดในหัวข้อของแต่ละ sub-phase)

**งานถัดไปที่ควรทำ (เรียงตามลำดับ):**
1. **ลงมือ Phase 5e** (configurable system settings: `UPLOAD_BASE_PATH`, max upload size, ค่าองค์กร) ตาม `phase5-plan.md`
...
5. **ของค้าง #13** — role `ADMIN` ธรรมดาดาวน์โหลด/พรีวิวไฟล์ไม่ได้เลย (403) เพราะ `routeAcceptted('user')` ไม่รวม `'admin'` — ไม่บล็อก 5e แต่ควรแก้ก่อนเปิดใช้งานจริงกับ role นี้
