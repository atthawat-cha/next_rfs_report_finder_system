...
**Phase 5 วางแผนแล้ว (2026-08-19), เริ่มลงมือแล้ว** — โจทย์มาจาก `document/diff_req.md` ที่ผู้ใช้ commit เข้ามาเอง (`beb62ee`) แผนเต็มอยู่ที่ [`phase5-plan.md`](./phase5-plan.md) แบ่ง 6 sub-phase (5a-5f) ตัดสินใจครบทุกข้อกับผู้ใช้แล้วก่อนเขียน (ดูหัวข้อ "Resolved decisions" ในแผน) — **5a, 5b ปิดจบแล้ว (2026-08-19)** ดูรายละเอียดใต้ตาราง Phase 5 ด้านล่าง (5b เจอว่าแผนเดิมเข้าใจผิดว่า per-report ACL "zero UI" — ที่จริงมี inline UI มาตั้งแต่ 2d แล้ว ดูรายละเอียดในหัวข้อ 5b)

**งานถัดไปที่ควรทำ (เรียงตามลำดับ):**
1. **ลงมือ Phase 5c** (เติมหน้า `/permissions` stub + `GET`/`PUT /api/users/roles/[id]`) ตาม `phase5-plan.md`
...
5. **ของค้าง #13** — role `ADMIN` ธรรมดาดาวน์โหลด/พรีวิวไฟล์ไม่ได้เลย (403) เพราะ `routeAcceptted('user')` ไม่รวม `'admin'` — ไม่บล็อก 5c แต่ควรแก้ก่อนเปิดใช้งานจริงกับ role นี้
...
| **5f** | Housekeeping: refresh `feature-list.md` (ค้างอีกรอบ), `docker-compose.yml` (Redis), แก้ lint error ทั้งหมด (37 ตัว ณ หลัง 5b — ดูของค้าง #11) + ใส่ `--max-warnings 192` ratchet เข้า CI | ⬜ | — |
