...
**Phase 5 วางแผนแล้ว (2026-08-19), เริ่มลงมือแล้ว** — โจทย์มาจาก `document/diff_req.md` ที่ผู้ใช้ commit เข้ามาเอง (`beb62ee`) แผนเต็มอยู่ที่ [`phase5-plan.md`](./phase5-plan.md) แบ่ง 6 sub-phase (5a-5f) ตัดสินใจครบทุกข้อกับผู้ใช้แล้วก่อนเขียน (ดูหัวข้อ "Resolved decisions" ในแผน) — **5a, 5b, 5c, 5d, 5e ปิดจบแล้ว (2026-08-19)** ดูรายละเอียดใต้ตาราง Phase 5 ด้านล่าง (5b เจอว่าแผนเดิมเข้าใจผิดว่า per-report ACL "zero UI" — ที่จริงมี inline UI มาตั้งแต่ 2d แล้ว; 5c เจอบั๊กจริง 2 ชั้นใน `buildRolePermissionInsert` ที่ทำให้ submenu-level permission grant ไม่เคยบันทึกได้เลยตั้งแต่ 2d แก้แล้ว; 5d เจอว่า `dialog-drawer.tsx`/`right-drawer.tsx` (ทั้งคู่) เป็น scaffold ที่ใช้งานจริงไม่ได้; 5e เจอ raw link ใน `/shares/[token]` ที่จะพังทันทีที่เปลี่ยน `UPLOAD_BASE_PATH` แก้แล้ว — ดูรายละเอียดในหัวข้อของแต่ละ sub-phase)

**งานถัดไปที่ควรทำ (เรียงตามลำดับ):**
1. **ลงมือ Phase 5f** (housekeeping: refresh `feature-list.md`, `docker-compose.yml`, แก้ lint error 41 ตัว + ratchet CI) ตาม `phase5-plan.md` — sub-phase สุดท้ายของ Phase 5
...
5. **ของค้าง #13** — role `ADMIN` ธรรมดาดาวน์โหลด/พรีวิวไฟล์ไม่ได้เลย (403) เพราะ `routeAcceptted('user')` ไม่รวม `'admin'` — ยังไม่แก้ ควรพิจารณาก่อนเปิดใช้งานจริงกับ role นี้
...
**อัปเดต (2026-08-19, ระหว่าง 5e):** ขยับเล็กน้อยเป็น **41 error / 199 warning (รวม 240)** — error ไม่เพิ่มเลย, +2 warning จาก `no-unused-expressions` idiom เดิมในไฟล์ route ใหม่ 2 ไฟล์ (`settings/public`, `shares/[token]/files/[fileId]/download`) — ส่วนที่แก้ได้ถูกๆ (unused `_req` param ที่ไม่จำเป็น) แก้ก่อน commit แล้ว **ตัวเลขที่ 5f ต้องปิดให้ครบตอนนี้คือ 41 error ไม่ใช่ 36/37/38**
