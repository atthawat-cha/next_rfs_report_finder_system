...
>
> **อัปเดตแบบ spot-check 2026-08-28** (ไม่ใช่การไล่ครบ 94 แถวใหม่ทั้งหมด — เทียบเฉพาะแถวที่ Phase 12 กระทบจริงกับ `00-progress.md`/`git log` ล่าสุด): พบ 2 แถวที่ค้าง (ตัวเลขในตารางไม่ผิด แต่ justification text อ้างสถานะเก่าก่อน Phase 12) — แถว "Automated test suite" (§16 Security & Compliance) ยังเขียนว่า "E2E ยังไม่ทำ" ทั้งที่ Phase 12a/follow-up (`d2c6c44`, `0fd1581`) เพิ่ม Playwright E2E จริงแล้ว 8/8 spec; แถว "Structured logging + error tracking" ยังเขียนว่าไม่มี error-tracking vendor ทั้งที่ Phase 12c (`93c648a`) ต่อ Sentry จริงแล้ว (env-gated) — แก้ทั้งคู่ในรอบนี้ นอกจากนี้พบว่า PDF inline preview (แถว §5) เพิ่งถูกยืนยันว่าเคย**พังจริงทั้งระบบ**มาตั้งแต่เริ่มมี (CSP `object-src 'none'` บล็อก `<embed>` ทุกจุดเงียบๆ ไม่มีใครสังเกตเพราะไม่เคยมี browser tool ทดสอบจริงจนกว่าจะมี Playwright ใน 12a) เพิ่งแก้จบจริงเมื่อ 2026-08-27 (`740a43b`/`df964b1`/`cace2fd`) — อัปเดต justification ไว้ด้วย ไม่เปลี่ยนสัญลักษณ์ (เดิมก็ ✅ อยู่แล้วในเอกสาร แม้โค้ดจริงจะพังมาตลอด — เป็นช่องว่างระหว่างเอกสารกับพฤติกรรมจริงที่ควรระวังไว้เป็นบทเรียน)
>
...
| Structured logging + error tracking (pino/Sentry) | Should | ✅ (7a mass-replace `console.*` → `lib/logger.ts`'s pino จริงครบทุก API route handler + `lib/*` ที่เกี่ยวข้อง รวม `logDevError` เองก็ต่อ pino แล้วนอก dev — เหลือแค่ client component console.* ที่ตั้งใจไม่แตะเพราะ pino เป็น Node-only; **12c (`93c648a`) ต่อ `@sentry/nextjs` จริงแล้ว** ผ่าน `instrumentation.ts`/`instrumentation-client.ts`, hook เข้า `logDevError()` ครบทุก call site, env-gated ด้วย `SENTRY_DSN`/`NEXT_PUBLIC_SENTRY_DSN` (ไม่ตั้ง = no-op จริง ยืนยันแล้ว) — live error-capture ยังไม่เคยยืนยันเพราะไม่มี DSN จริงให้ผู้ใช้ตั้ง เป็น follow-up ที่ผู้ใช้ต้องทำเอง) | 4f/7a/12c |
| Automated test suite (unit/integration/E2E) | Must | ✅ (Vitest, `lib/report-acl.test.ts` 7 test — integration ต่อ dev DB จริง; **12a (`d2c6c44`) เพิ่ม Playwright E2E จริงแล้ว** — golden-path spec 4 ไฟล์ (auth/report-search/report-admin-crud/locale-switch) + follow-up `report-editor-tabs.spec.ts` (`0fd1581`) รวม 8/8 spec ผ่านจริงทั้งกับ dev server และ fresh build) | 4b/12a |
