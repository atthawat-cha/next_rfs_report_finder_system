...
**สถานะ: ✅ เสร็จแล้ว** (`20260817182535_add_user_theme_preference`) — ปลดบล็อกเมื่อ `.env`/`.env.local` ถูกชี้ไปที่ DB คนละตัว (`nextjs_rfs`@`localhost:5432`) แทน `next_rfs_master`@`5434` เดิม ระหว่าง session นี้ ตรวจแล้วว่า **drift เดิมยังอยู่** (แค่ย้ายไป DB ใหม่) — `npx prisma migrate dev --name add_user_theme_preference` ยัง fail ด้วย drift signature เดิมเป๊ะ (`menus_permissions` หาย, `users.role_id`/`permissions.menu_id` เพิ่มมาไม่มี migration รองรับ, index ของ `reports` ไม่ตรง) และ `_prisma_migrations` ของ DB ใหม่มี `applied_steps_count = 0` ทั้ง 4 แถว → ยืนยันว่ามีคนรัน **Option B** (เก็บข้อมูลเดิม, `migrate resolve --applied`) ไปแล้วกับ DB นี้ก่อนหน้า session นี้ โดยไม่ได้อัปเดตเอกสาร

ทำตาม pattern เดียวกันสำหรับคอลัมน์ใหม่ (ไม่ใช้ `migrate reset` เพราะจะลบข้อมูล seed ที่มีอยู่ 4 users/5 reports โดยไม่จำเป็น): เขียน `migration.sql` มือ (`ALTER TABLE users ADD COLUMN theme_preference TEXT`) → รันตรงกับ DB ผ่าน `psql` → `prisma migrate resolve --applied` → `prisma generate` → `prisma migrate status` ยืนยัน "up to date" ของค้าง DB drift เดิม (ของค้าง #1 ใน `00-progress.md`) **ยังไม่ถูกแก้จริง** เป็นแค่ baseline ทับไว้ ยังต้องตัดสินใจ/สื่อสารกับทีมเรื่อง root cause อยู่ดี
