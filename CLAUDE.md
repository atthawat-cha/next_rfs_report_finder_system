...
Route-group naming caution: because Next.js strips `(auth)` from the actual URL, a page at `app/(auth)/dashboard/page.tsx` is served at `/dashboard`, **not** `/(auth)/dashboard`. `proxy.ts` (renamed from `middleware.ts` in Phase 8, per Next.js 16's Middleware→Proxy deprecation — see `document/phase8-plan.md`) has no `protectedPaths` concept (Phase 0 removed it, `5e799c1`) — the real gate is: `publicPaths = ['/login', '/']` plus any `/shares/`-prefixed path skip the auth check outright, and the `matcher` config runs `proxy()` on everything else except `/api/*`, `_next/static`, `_next/image`, and `favicon.ico`. Anything not in `publicPaths` that reaches `proxy()` without a decoded user gets redirected to `/login`. Verify against `matcher` and `proxy()` itself, not against a `protectedPaths` list — it doesn't exist. Note also that `proxy.ts` runs on the **Node.js runtime** (Next 16's Proxy defaults to it, with no way to opt back into Edge) — `middleware.ts` used to run on Edge, which is why some `lib/*` files' comments still explain a "kept off Edge" constraint that's now historical, not enforced.

### Auth model

- JWT-based, via `jose` (`lib/auth.ts`), stored in an httpOnly cookie named `auth-token` (`COOKIE_NAME` in `lib/auth.ts` — this is the only auth cookie name in the codebase; `proxy.ts` does not read or log any `auth_token` cookie).
...
- shadcn/ui (`style: new-york`, `baseColor: neutral`) — primitives in `components/ui/`, composed feature components in `components/shared/` (`dataTable.tsx`, `dialog-drawer.tsx`, `searchInput.tsx`, `permissions-form.tsx`, `fileuploading.tsx`) and `components/layouts/` (sidebar/navbar/menu shell). `dialog-drawer.tsx`'s `DrawerDialogDemo` is unsuited as a *reference* to copy for a new dialog — it ignores the `isOpen` prop it's given and can't be seeded with existing data — which is why controlled dialogs (`ReportPreviewDialog`, `ReportPermissionsDrawer`, `MenuFormDialog`, and since Phase 7b `CategoryFormDialog`/`TagFormDialog`) are written from scratch instead of extending it. As of Phase 7b it is a real dependency of exactly one page, `user-management/user-department` — `reports/categories`/`reports/tags` were migrated off it (see next bullet).
- Tables use `@tanstack/react-table` — the pattern is a `*Column.tsx` (column defs, as a factory function taking `onEdit`/`onDelete` callbacks) + `*Table.tsx`/`*MainTable.tsx` (page-level data fetch + state) + a bare TanStack renderer (`SharedDataTable` from `components/shared/dataTable.tsx`, or a page-local equivalent like `catagoriesTable.tsx`/`tagsTable.tsx`) for rendering. See `app/(auth)/settings/menus/` (`menusColumn.tsx` + `MenuFormDialog` + `DeleteMenuDialog`) or, since Phase 7b, `app/(auth)/reports/categories/components/`/`.../tags/components/` for the reference implementation to copy when adding a new manageable list — **not** the pre-7b versions of those same two pages, which were non-functional stubs (hardcoded `data={[]}`, no backing API, create forms that only `console.log`) despite this file previously pointing at them; see `document/00-progress.md`'s Phase 7 notes.
...
7. **Extra care** (don't skip verification steps) when touching: auth/session (`lib/auth.ts`, `proxy.ts`), the ACL layer (`lib/report-acl.ts`, `report_permissions`), file upload/path handling (`lib/fileUploadServices.ts`), or any Prisma schema change (`prisma/schema.prisma` + migration) — these are the areas most likely to introduce a real security/data bug versus a cosmetic one.
